...
@app begin
    ...
    @out codes = soil_data.fips
    @in selected_code = 1001
    ...
    @out station_data = DataFrame("PS_date" => Date[], "PS" => Float64[], "TS_date" => Date[], "TS" => Float64[], "T2M_date" => Date[], "T2M" => Float64[], "WS10M_date" => Date[], "WS10M" => Float64[], "PRECTOT_date" => Date[], "PRECTOT" => Float64[])
    @in N = 400
    # map plot data, plot configured in GB
    @out station_lat = soil_data.lat
    @out station_lon = soil_data.lon
    @out mapcolor = []
    ...
    @onchange selected_code, selected_metrics, N, isready begin
        processing = true
        station_data[!] = DataFrame() # with the [!] suffix, the reactive variable changes in value but the update is not sent to the browser
    ...
    @onchange data_click begin
        ...
        if haskey(data_click, "points")
            ...
            selected_code = codes[closest_index]
        else
            ...
            date_data = df[(df.date .== Date(date)) .& (df.fips .== selected_code), selected_metrics] |> Matrix |> vec
